...
from select_component.select import select
from table_component.table import table
...
def get_all_schema(BASE_URL: str, fieldPath: str, start_date, end_date) -> pd.DataFrame:
    ...
    query = getattr(protocol.Query, fieldPath)(first=100000, where={
        'timestamp_gte': start_date, 'timestamp_lte': end_date})
...
df_markets['inputToken'] = pd.DataFrame(
    [*df_markets['inputToken']])[['symbol']]
df_markets.set_index('inputToken', inplace=True)
df_markets['totalDepositBalanceUSD'] = df_markets['totalDepositBalanceUSD'].astype(
    'double')
df_markets['totalBorrowBalanceUSD'] = df_markets['totalBorrowBalanceUSD'].astype(
    'double')
df_markets['Available to Borrow (USD)'] = df_markets['totalDepositBalanceUSD'] - \
    df_markets['totalBorrowBalanceUSD']


dfm2 = get_all_schema(
    BASE_URL, 'usageMetricsDailySnapshots', start_date, end_date)
df_usageMetrics = dfm2.copy()
df_usageMetrics.set_index('usageMetricsDailySnapshots_timestamp', inplace=True)
df_usageMetrics.index = pd.to_datetime(
    df_usageMetrics.index, unit='s').to_period('D').to_timestamp()
df_usageMetrics.sort_index(inplace=True)


dfm3 = get_all_schema(
    BASE_URL, 'financialsDailySnapshots', start_date, end_date)
df_financials = dfm3.copy()
df_financials.set_index('financialsDailySnapshots_timestamp', inplace=True)
df_financials.index = pd.to_datetime(
    df_financials.index, unit='s').to_period('D').to_timestamp()
...
markets_snapshots = markets_snapshots.astype({'timestamp': 'int',
                                             'totalValueLockedUSD': 'double',
                                              ...
                                              'exchangeRate': 'double'})
markets_snapshots['timestamp'] = pd.to_datetime(
    markets_snapshots['timestamp'], unit='s').dt.to_period('D').dt.to_timestamp()
markets_snapshots.set_index('timestamp', inplace=True)
markets_snapshots.sort_index(inplace=True)
markets_snapshots = pd.concat([pd.json_normalize(markets_snapshots['market']).set_index(
    markets_snapshots.index), markets_snapshots.drop('market', axis=1)], axis=1)
...
with column1:
    ...
    df1.rename(
        columns={'totalDepositBalanceUSD': 'Total Deposited (USD)'}, inplace=True)
...
with column2:
    ...
    df2.rename(
        columns={'totalBorrowBalanceUSD': 'Total Borrowed (USD)'}, inplace=True)
...
with column1:
    df4 = df_usageMetrics.loc[:, [
        'usageMetricsDailySnapshots_dailyTransactionCount', 'usageMetricsDailySnapshots_dailyActiveUsers']]
    df4 = df4.resample(g4).sum()
    df4.rename(columns={
               'usageMetricsDailySnapshots_dailyTransactionCount': 'Daily Transaction Count'}, inplace=True)
    df4.rename(columns={
               'usageMetricsDailySnapshots_dailyActiveUsers': 'Daily Unique Active User'}, inplace=True)

    fig4 = go.Figure(data=[
        go.Bar(name='Daily Transaction Count', x=df4.index,
               y=df4.loc[:, 'Daily Transaction Count']),
        go.Bar(name='Daily Unique Active User', x=df4.index,
               y=df4.loc[:, 'Daily Unique Active User'])
    ])
...
with column1:
    ...
    fig6 = go.Figure(data=[
        go.Bar(name='Protocol Side Revenue (USD)', x=df6.index,
               y=df6.loc[:, 'Protocol Side Revenue (USD)']),
        go.Bar(name='Supply Side Revenue (USD)', x=df6.index,
               y=df6.loc[:, 'Supply Side Revenue (USD)']),
        go.Scatter(name='Total Revenue (USD)', x=df6.index,
                   y=df6.loc[:, 'Total Revenue (USD)'])
    ])
...
with column3:
    ...
    df7['Utilization (%)'] = df7.loc[:, 'Outstanding Loans (USD)'] / \
        df7.loc[:, 'Outstanding Deposits (USD)']
    df7 = df7.resample(g7).last()

    fig7 = make_subplots(specs=[[{"secondary_y": True}]])
    fig7.add_trace(go.Scatter(name='Outstanding Deposits (USD)', x=df7.index,
                   y=df7['Outstanding Deposits (USD)'], mode='lines', fill='tozeroy'), secondary_y=False)
    fig7.add_trace(go.Scatter(name='Outstanding Loans (USD)', x=df7.index,
                   y=df7['Outstanding Loans (USD)'], mode='lines', fill='tozeroy'), secondary_y=False)
    fig7.add_trace(go.Scatter(name='Utilization (%)', x=df7.index,
                   y=df7['Utilization (%)']), secondary_y=True)
    ...
    fig7.update_yaxes(
        title_text='Outstanding Deposits and Loans (USD)', secondary_y=False)
...
with column1:
    ...
    df8['Net Change (USD)'] = df8.loc[:, 'Deposits (USD)'] - \
        df8.loc[:, 'Withdraws (USD)']
    df8['Withdraws (USD)'] = -df8.loc[:, 'Withdraws (USD)']
    df8a = df8.loc[:, [
        'Deposits (USD)', 'Withdraws (USD)', 'Net Change (USD)']].resample(g8).sum()
    df8b = df8.loc[:, ['Outstanding Deposits (USD)']].resample(g8).last()
    df8 = pd.concat([df8a, df8b], axis=1)

    fig8 = make_subplots(specs=[[{"secondary_y": True}]])
    fig8.add_trace(go.Bar(name='Deposits (USD)', x=df8.index,
                   y=df8.loc[:, 'Deposits (USD)']), secondary_y=False)
    fig8.add_trace(go.Bar(name='Withdraws (USD)', x=df8.index,
                   y=df8.loc[:, 'Withdraws (USD)']), secondary_y=False)
    fig8.add_trace(go.Scatter(name='Net Change (USD))', x=df8.index,
                   y=df8.loc[:, 'Net Change (USD)'], mode='markers'), secondary_y=False)
    fig8.add_trace(go.Scatter(name='Outstanding Deposits (USD)', x=df8.index,
                   y=df7.loc[:, 'Outstanding Deposits (USD)']), secondary_y=True)
    ...
    fig8.update_layout(
        ...
        xaxis_title='Time')
    fig8.update_yaxes(
        title_text='Deposits and Withdraws (USD)', secondary_y=False)
    fig8.update_yaxes(
        title_text='Outstanding Deposits (USD)', secondary_y=True)
...
with column3:
    ...
    df9.rename(columns={
        ...
        'financialsDailySnapshots_totalBorrowBalanceUSD': 'Outstanding Loans (USD)'}, inplace=True)
    df9['Net Change (USD)'] = df9.loc[:, 'Borrows (USD)'] - \
        df9.loc[:, 'Repays (USD)']
    df9['Repays (USD)'] = -df9.loc[:, 'Repays (USD)']
    df9a = df9.loc[:, [
        'Borrows (USD)', 'Repays (USD)', 'Net Change (USD)']].resample(g9).sum()
    df9b = df9.loc[:, ['Outstanding Loans (USD)']].resample(g9).last()
    df9 = pd.concat([df9a, df9b], axis=1)

    fig9 = make_subplots(specs=[[{"secondary_y": True}]])
    fig9.add_trace(go.Bar(name='Borrows (USD)', x=df9.index,
                   y=df9.loc[:, 'Borrows (USD)']), secondary_y=False)
    fig9.add_trace(go.Bar(name='Repays (USD)', x=df9.index,
                   y=df9.loc[:, 'Repays (USD)']), secondary_y=False)
    fig9.add_trace(go.Scatter(name='Net Change (USD))', x=df9.index,
                   y=df9.loc[:, 'Net Change (USD)'], mode='markers'), secondary_y=False)
    fig9.add_trace(go.Scatter(name='Outstanding Loans (USD)', x=df9.index,
                   y=df7.loc[:, 'Outstanding Loans (USD)']), secondary_y=True)
...
with column1:
    ...
    df10 = df10.groupby('Markets').resample(g10).sum(
    ).loc[:, ['Liquidations (USD)']].reset_index('Markets')
    top_markets = df10.groupby('Markets').max().sort_values(
        'Liquidations (USD)', ascending=False).head()
    df10.loc[~df10.loc[:, 'Markets'].isin(
        top_markets.index), 'Markets'] = 'Other'
    df10 = df10.groupby(['timestamp', 'Markets']).sum().swaplevel(axis=0)
    graph_order = df10.groupby('Markets').max().sort_values(
        'Liquidations (USD)', ascending=False)

    fig10 = go.Figure()
    for token in graph_order.index:
        fig10.add_trace(go.Bar(
            name=token, x=df10.loc[token].index, y=df10.loc[token, 'Liquidations (USD)']))
...
with column1:
    ...
    top_markets = df12.groupby('id').max().sort_values(
        'Total Deposited (USD)', ascending=False).head()
    df12.loc[~df12.loc[:, 'id'].isin(top_markets.index), 'Markets'] = 'Other'
    df12 = df12.groupby(['timestamp', 'Markets']).sum().swaplevel(axis=0)
    graph_order = df12.groupby('Markets').max().sort_values(
        'Total Deposited (USD)', ascending=False)

    fig12 = go.Figure()
    for token in graph_order.index:
        fig12.add_trace(go.Scatter(
            name=token, x=df12.loc[token].index, y=df12.loc[token, 'Total Deposited (USD)'], stackgroup='one', hoverinfo='x+y'))
...
with column3:
    ...
    graph_order = df13.groupby('Markets').max().sort_values(
        'Total Loans (USD)', ascending=False)

    fig13 = go.Figure()
    for token in graph_order.index:
        fig13.add_trace(go.Scatter(
            name=token, x=df13.loc[token, :].index, y=df13.loc[token, 'Total Loans (USD)'], stackgroup='one', hoverinfo='x+y'))
...
with column1:
    ...
    df14 = df14.melt(ignore_index=False).pivot_table(index=['timestamp', 'id', 'Markets'], columns=[
        None]).droplevel(level=0, axis=1).reset_index(['id', 'Markets'])
    df14['Utilization (%)'] = df14.loc[:, 'Total Borrow (USD)'] / \
        df14.loc[:, 'Total Deposited (USD)']
    df14 = df14.groupby(['id', 'Markets'], as_index=False).resample(
        g14).last().droplevel(level=0, axis=0)

    fig14 = go.Figure()
    for token_address in filter(None, df14['id'].unique()):
        fig14.add_trace(go.Scatter(name=df14.loc[df14['id'] == token_address]['Markets'][0], x=df14.loc[df14['id']
                        == token_address].index, y=df14.loc[df14['id'] == token_address]['Utilization (%)'].tolist()))
...
with column1:
    df15 = markets_snapshots[['inputToken.symbol', 'id', 'rates']].set_index(
        ['inputToken.symbol', 'id'], append=True)
    ...
    for x in df15.columns:
        ...
        tmp_df = tmp_df.rename(
            {'rate': tmp_df.iloc[0, 1]}, axis=1).drop('side', axis=1)
        df_rates = pd.concat([df_rates, tmp_df], axis=1)
    df15 = df_rates.reset_index(['id', 'inputToken.symbol'])
    df15 = pd.pivot_table(df15, index=['timestamp'], columns=[
                          'id', 'inputToken.symbol'])
    df15 = df15.fillna(method='ffill')
    df15 = df15.melt(ignore_index=False).pivot_table(index=['timestamp', 'id', 'inputToken.symbol'], columns=[
        None]).droplevel(level=0, axis=1).reset_index(['id', 'inputToken.symbol'])
    df15 = df15.groupby(['id', 'inputToken.symbol'], as_index=False).resample(
        g15).last().droplevel(level=0, axis=0)
    df15.sort_index(inplace=True)

    fig15a = go.Figure()
    for token_address in filter(None, df15['id'].unique().tolist()):
        fig15a.add_trace(go.Scatter(name=df15.loc[df15['id'] == token_address].iloc[0, 1],
                         x=df15.index, y=df15.loc[df15['id'] == token_address].iloc[:, 2], mode='lines'))
...
with column2:
    fig15b = go.Figure()
    for token_address in filter(None, df15['id'].unique().tolist()):
        fig15b.add_trace(go.Scatter(name=df15.loc[df15['id'] == token_address].iloc[0, 1],
                         x=df15.index, y=df15.loc[df15['id'] == token_address].iloc[:, 3], mode='lines'))
